...
export default function SampleBuilder({ snapshot, onChange }: Props) {
  ...
  function apply(action: () => Snapshot, message: string) {
    try {
      ...
      setError(problem instanceof Error ? problem.message : 'This portfolio change could not be applied.');
    }
  }
  ...
  return <div className={styles.builder}>
    <div className={styles.toolbar}>
      ...
      <div className={styles.quote}>
        <label htmlFor="sample-quote">Denomination</label>
        <Select id="sample-quote" label="Denomination" value={quote}
    ...
    <div className={styles.rows}>
      {snapshot.positions.map(position => <PositionEditor
        ...
        onRemove={() => {
          apply(() => removeSamplePerp(snapshot, position.id), `${position.market} removed from the portfolio.`);
          addButton.current?.focus();
        }}
      />)}
    </div>
    {!snapshot.positions.length && <div className={styles.empty}><strong>Your portfolio is ready for a fresh start.</strong><p>Add a perp to explore its price exposure.</p></div>}
    ...
    <dialog ref={catalog} className={styles.catalog} aria-labelledby="perp-catalog-title"
      ...
      <div className={styles.catalogHeading}>
        <div><span className="eyebrow">YOUR PORTFOLIO</span><h2 id="perp-catalog-title">Add perpetuals</h2></div>
        ...
        <div className={styles.catalogList}>
          {filtered.map(market => {
            ...
            return <button key={market.marketIndex} className={styles.catalogMarket} type="button" disabled={added}
              ...
              onClick={() => apply(() => addSamplePerp(snapshot, market.asset, quote), `${market.market} added to the portfolio.`)}>
      ...
      <div className={styles.catalogFooter}><span role="status">{snapshot.positions.length} perps in your portfolio · {quote}</span><button className="button primary" type="button" onClick={closeCatalog}>Done</button></div>
    </dialog>
  </div>;
}
...
function PositionEditor({ position, onApply, onRemove }: {
  ...
  return <form className={styles.position} aria-label={`Edit ${position.market}`} onSubmit={submit} noValidate>
    <div className={styles.positionHeading}>
      ...
      <button className={styles.remove} aria-label={`Remove ${position.market}`} title="Remove position" type="button" onClick={onRemove}><Icon name="close" size={17} /></button>
    </div>
    {editable ? <>
      <div className={styles.fields}>
        ...
        <div><label htmlFor={`${inputId}-price`}>Reference price · {position.quote}</label><input id={`${inputId}-price`} inputMode="decimal" maxLength={37} autoComplete="off" value={input.price} aria-invalid={Boolean(error)} aria-describedby={error ? `${inputId}-error` : undefined} onChange={event => { setInput({ ...input, price: event.target.value }); setError(''); }} /></div>
      </div>
