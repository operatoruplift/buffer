...
const FIXTURE_REASON = 'Editable sample baseline price. No live oracle was read.';
...
function sampleQuote(value: string): SampleQuote {
  if (!SAMPLE_QUOTES.some(quote => quote === value)) throw new Error('Choose USDC, USDT, or USD for this sample.');
...
function fixtureMetrics(positions: Position[], quote: SampleQuote): Snapshot['metrics'] {
  ...
  return [
    { label: 'Gross notional', value: long.plus(short).toFixed(), unit: quote, explanation: 'Modeled sample positions only; excluded positions omitted. Absolute quantity × baseline price in this denomination; exposure, not equity.' },
    { label: 'Long notional', value: long.toFixed(), unit: quote, explanation: 'Modeled sample positions only; excluded positions omitted. Long exposure stays fixed when you move the scenario slider.' },
    { label: 'Short notional', value: short.toFixed(), unit: quote, explanation: 'Modeled sample positions only; excluded positions omitted. Absolute short exposure; no collateral or margin health is assumed.' },
  ];
}
...
function makePosition(asset: string, input: SamplePerpInput, quote: SampleQuote): Position {
  ...
  if (!market) throw new Error('Choose a perpetual from the sample market catalog.');
...
/** A new detached sample; neither this factory nor the editor reads a provider. */
export function getPortfolioSampleSnapshot(): Snapshot {
  ...
  return {
    ...
    warnings: ['Sample portfolio: all quantities and baseline prices are editable fixtures. No account, collateral balance, or trade is created.'],
    provenance: [
      'Buffer sample portfolio builder. No wallet address, live account, or RPC slot is associated with this fixture.',
      ...
      'USDC, USDT, and USD are illustrative sample denominations. Changing denomination preserves numeric inputs, performs no currency conversion, and does not assert venue settlement support.',
    ],
  };
}
...
function editableSnapshot(snapshot: Snapshot): Snapshot {
  if (snapshot.source !== 'sample' || snapshot.network !== 'fixture' || snapshot.authority !== null ||
    ...
    throw new Error('Only sample portfolios can be edited. Live account snapshots are read only.');
  }
  ...
  next.positions = next.positions.map(position => {
    ...
    if (!market) return { ...position, modeled: false, exclusionReason: position.exclusionReason ?? 'This market is outside the sample catalog.' };
...
export function addSamplePerp(snapshot: Snapshot, asset: string, value = getSampleQuote(snapshot)): Snapshot {
  const next = editableSnapshot(snapshot);
  if (next.positions.some(position => position.asset === asset)) throw new Error('This perpetual is already in your sample. Edit its existing position.');
  const market = SAMPLE_MARKET_CATALOG.find(entry => entry.asset === asset);
  if (!market) throw new Error('Choose a perpetual from the sample market catalog.');
...
export function updateSamplePerp(snapshot: Snapshot, id: string, input: SamplePerpInput): Snapshot {
  ...
  if (index < 0) throw new Error('This position is no longer in your sample.');
...
export function removeSamplePerp(snapshot: Snapshot, id: string): Snapshot {
  const next = editableSnapshot(snapshot);
  if (!next.positions.some(position => position.id === id)) throw new Error('This position is no longer in your sample.');
