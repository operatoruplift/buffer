...
export function normalizeSnapshot(input: ReadData): Snapshot {
  ...
  const positions = account.perpPositions.filter((p) => !positionIsAvailable(p) || !p.openBids.isZero() || !p.openAsks.isZero()).map((position) => {
    ...
    const identity = Boolean(market && config && market.marketIndex === position.marketIndex && config.symbol.endsWith('-PERP') && config.symbol === marketName && config.oracle.equals(market.amm.oracle) && JSON.stringify(config.oracleSource) === JSON.stringify(market.amm.oracleSource));
  ...
  return { source: 'live', network: 'mainnet-beta', authority: input.authority, sampleName: null,
    ...
    provenance: [`Solana mainnet-beta · confirmed commitment · Drift SDK ${dependencies['@drift-labs/sdk']}.`,
      ...
      'All modeled perpetual identities are checked against pinned mainnet configuration, decoded metadata, oracle address/source, and the fixed Drift program. Quote currencies are checked by quote-market index, name, and mint.'] };
}
