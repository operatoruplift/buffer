...
const PROGRAM = new PublicKey(DRIFT_PROGRAM_ID);
...
export const liveProvider: LiveProvider = {
  discover: (authority) => withScope(authority, async (scope) => {
    ...
    return { authority, subaccounts: accounts.map((account) => subaccountInfo(account, getUserAccountPublicKeySync(PROGRAM, scope.authority, account.subAccountId).toBase58())).sort((a, b) => a.id - b.id), retrievedAt: new Date().toISOString() };
  }),
