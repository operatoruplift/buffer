...
| Live API capture | Fresh fetches to the production alias, `2026-09-15T07:35:19.791Z`, with public example authorities only. No credentials were printed or stored. |
...
| **R10 — Risk context** | Show current provider health/maintenance observations with method/scope/limits; never present an invented exact liquidation forecast. | **Verified locally; exact liquidation estimate explicitly deferred.** Velocity SDK cross-margin health and provider-reported margin values are labeled as observations, not forecasts; isolated positions remain separate. Jupiter inventory and Pacifica limitations are surfaced. | Normalizer tests cover health/oracle confidence/lag and exclusion reasons; UI method text lists collateral, funding, fees, borrowing, future fills, and liquidation exclusions. | App method, health, and exclusion surfaces. | A verified collateral/oracle/liquidation contract for every supported venue is required before an exact model can be implemented. |
| **R11 — Durable alerts** | If advertised, prove rule → fresh evaluation → outbox/event → worker → delivery, including owner isolation, leases, idempotency, cooldown, retries, pause/delete, and stale suppression. | **Explicitly deferred conditional scope.** No durable alert tables, worker, delivery destination, or success claim exists in this revision; no inert alert control is advertised. | Absence verified by source/schema inventory; no partial monitor is presented as running. | No alert UI is rendered. | Add verified metric contract, additive Supabase migrations/RLS, scheduled worker, destination/secret, mock sink, idempotency/concurrency tests, and an authorized delivery channel. |
...
| `npm run lint` | Pass. |
| `npm test` | **367 tests passed**, 16 files. |
| Focused browser journeys | **26 passed** locally, covering scenarios, app/auth/demo, reports, PWA, and failure paths. |
...
| Production provider reads | Velocity accounts/snapshot 200 with 2 positions; Pacifica accounts/snapshot 200 with 22 positions; Jupiter accounts/snapshot 200 with `inventoryAvailable:false`; the inventory row count is live and can change between reads. |
...
| Durable alerts | Explicitly deferred conditional scope; no monitor is running. |
...
2. **Alerts (if authorized later):** verify a stable fresh metric contract per provider, add additive Supabase tables and owner-only RLS, implement a scheduled worker and durable outbox, configure an authorized destination/secret, and run mock-sink tests for leases, retries, idempotency, cooldown/hysteresis, deletion, and stale-input suppression.
...
The audited product is working for the committed read-only scenario, provider-read, report, media, PWA, and privacy flows. The matrix above intentionally keeps email delivery, durable alerts, exact liquidation forecasts, and native packages in their correct configuration-blocked or deferred states rather than presenting them as finished capabilities.
