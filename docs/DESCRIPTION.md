...
- **A reviewable threshold monitor.** Configure one owner-scoped headroom rule, run a fresh check, and inspect the durable local event, outbox, lease, and mock-delivery state. Guest demo state stays on the device; external destinations and hosted workers remain gated until configured and verified.
